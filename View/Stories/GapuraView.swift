    ...
    var body: some View {
        ZStack{
            ...
            VStack{
                Spacer()
                HStack{
                    Image(gapuraStories[currentIndex].isTalking.halfImage)
                }
                .padding(.bottom, -300)
                RoundedRectangle(cornerRadius: 16)
                    ...
                    .overlay {
                        VStack{
                            HStack{
                                Text(gapuraStories[currentIndex].text)
        ...
        .onTapGesture {
            if isTapGestureEnabled {
                if currentIndex < gapuraStories.count - 1 {
                    currentIndex += 1
                    if gapuraStories[currentIndex].text == "" {
                    ...
                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) {
                        ...
                        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                            ...
                            matchManager.gameStatus = .storyBalaiDesa
                        }
                    }
                }
            }
